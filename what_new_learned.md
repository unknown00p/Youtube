...
Problem: the thing is we can use the single Database connection socket that we get when starting the server and connecting to database. BUT it can only support one query at a time, so if we get 10 users requesting database quering, first query will take (approx 100ms) to get done then second, then third and so on one by one. so the time taken for the 10th query will be 1000ms(10 times) more. which is straight up slow as turtle and if request grows more it will be disater
